...
class CollectionRegressionTestCase(unittest.IsolatedAsyncioTestCase):
    @staticmethod
    ...
    async def test_collections_uses_max_id_cursor_fallback(self):
        client = Client()
    ...
    async def test_collection_medias_chunk_uses_max_id_cursor_fallback(self):
        client = Client()
        client.private_request = AsyncMock(
            return_value={
        ...
        medias, next_max_id = await client.collection_medias_v1_chunk("123")

        self.assertEqual([media.pk for media in medias], ["1"])
        self.assertEqual(next_max_id, "cursor-2")

    async def test_collection_medias_uses_max_id_cursor_fallback_for_pagination(self):
        client = Client()
        client.private_request = AsyncMock(
            side_effect=[
                {
        ...
        medias = await client.collection_medias("123", amount=2)
