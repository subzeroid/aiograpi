...
from aiograpi.types import User, UserShort
...
class ProfileGraphQLRegressionTestCase(unittest.IsolatedAsyncioTestCase):
    maxDiff = None

    def build_client(self):
        client = Client()
        client.authorization_data = {"ds_user_id": "42", "sessionid": "test-session"}
        client.public_request = AsyncMock(return_value={"data": {"user": deepcopy(PROFILE_USER)}})
        return client
    ...
    async def test_user_short_gql_uses_current_profile_query(self):
        client = self.build_client()

        user = await client.user_short_gql("123")
    ...
    async def test_user_info_v2_gql_uses_current_profile_query(self):
        client = self.build_client()

        user = await client.user_info_v2_gql("123")
