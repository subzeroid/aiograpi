...
from aiograpi.exceptions import ChallengeError, ChallengeRequired, ClientError, UnknownError
from aiograpi.mixins.attestation import USDID_REFRESH_MARGIN, USDID_REGISTRATION_CLIENT_DOC_ID
from aiograpi.mixins.bloks import AP_2SV_ENTRYPOINT
...
class DeviceAttestationRegressionTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_usdid_header_is_three_segments_signed_over_uuid_and_expiry(self):
    ...
    async def test_usdid_header_absent_until_key_is_generated_and_then_cached(self):
        client = self.build_client()
    ...
    async def test_usdid_identity_reuse_force_rotation_and_header_refresh(self):
        client = self.build_client()
        client.usdid_generate()
        ...
        with mock.patch("aiograpi.mixins.attestation.time.time", return_value=1000):
    ...
    async def test_usdid_registration_token_is_valid_es256_jws(self):
        client = self.build_client()
        client.usdid_generate()
    ...
    async def test_usdid_register_uses_app_428_document_and_family_device_id(self):
        client = self.build_client()
        ...
        with mock.patch.object(client, "private_graphql_www_request", return_value=response) as graphql_request:
            result = await client.usdid_register()
    ...
    async def test_usdid_register_returns_false_for_rejection_or_missing_payload(self):
        client = self.build_client()
        responses = [
        ...
        with mock.patch.object(client, "private_graphql_www_request", side_effect=responses):
            self.assertFalse(await client.usdid_register())
            self.assertFalse(await client.usdid_register())

        self.assertFalse(client.usdid_registered)

    async def test_attestation_create_stores_nonce_and_uses_app_scoped_device_id(self):
        client = self.build_client()
        ...
        with mock.patch.object(client, "private_request", return_value=response) as private_request:
            result = await client.attestation_create_android_keystore(domain="b.i.instagram.com")
    ...
    async def test_attestation_create_uses_default_domain_and_clears_missing_nonces(self):
        client = self.build_client()
        ...
        with mock.patch.object(client, "private_request", return_value={"status": "ok"}) as private_request:
            await client.attestation_create_android_keystore()
    ...
    async def test_attestation_params_match_accepted_keystore_error_state(self):
        client = self.build_client()
    ...
    async def test_usdid_settings_roundtrip_does_not_leak_between_clients(self):
        client = self.build_client()
    ...
    async def test_clearing_usdid_settings_removes_stale_session_header(self):
        client = self.build_client()
        client.usdid_generate()
    ...
    async def test_resetting_device_rotates_family_id_and_clears_bound_usdid(self):
        client = self.build_client()
        client.usdid_generate()
        client.usdid_registered = True
        client.private.headers.update(client.base_headers)
...
class CaaLoginRegressionTestCase(unittest.IsolatedAsyncioTestCase):
    def build_client(self):
        client = Client()
        client.uuid = "00000000-0000-4000-8000-000000000000"
        client.phone_id = "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"
    ...
    async def test_caa_login_public_signatures_preserve_positional_prefixes(self):
        client = self.build_client()
    ...
    async def test_process_client_data_extracts_server_issued_aac(self):
        client = self.build_client()
        ...
        with mock.patch.object(client, "bloks_async_action", return_value=self.aac_response(aac)) as action:
            result = await client.bloks_caa_login_process_client_data(domain="b.i.instagram.com")
    ...
    async def test_extract_aac_supports_initial_and_rejects_malformed_payloads(self):
        client = self.build_client()
        direct = {
            "layout": {
                "bloks_payload": {
    ...
    async def test_bloks_transport_forwards_login_domain_and_extra_headers(self):
        client = self.build_client()

        with mock.patch.object(client, "private_request", side_effect=[{"kind": "action"}, {"kind": "app"}]) as request:
            action_result = await client.bloks_async_action(
                "com.example.action",
                {"value": 1},
                domain="b.i.instagram.com",
            ...
            app_result = await client.bloks_app(
                "com.example.app",
                {"value": 2},
                domain="b.i.instagram.com",
                login=True,
    ...
    async def test_oauth_preflight_uses_username_aac_and_waterfall(self):
        client = self.build_client()
        client.caa_aac = "server-aac"

        with mock.patch.object(client, "bloks_async_action", return_value={"status": "ok"}) as action:
            await client.bloks_caa_login_oauth_token_fetch(
    ...
    async def test_prepare_runs_current_preflight_sequence(self):
        client = self.build_client()
        order = []
        ...
        with (
            ...
            result = await client.bloks_caa_login_prepare(
                username="override_user",
                domain="b.i.instagram.com",
                waterfall_id="waterfall-1",
        ...
        process_client_data.assert_awaited_once_with(
            waterfall_id="waterfall-1",
            offline_experiment_group="experiment-1",
            bloks_versioning_id="bloks-version-1",
            domain="b.i.instagram.com",
        )
        oauth_token_fetch.assert_awaited_once_with(
            username="override_user",
            waterfall_id="waterfall-1",
            offline_experiment_group="experiment-1",
            bloks_versioning_id="bloks-version-1",
    ...
    async def test_prepare_stops_when_usdid_registration_is_rejected(self):
        client = self.build_client()
    ...
    async def test_prepare_skips_registration_and_oauth_without_aac(self):
        client = self.build_client()
        client.usdid_registered = True
    ...
    async def test_prepare_returns_false_when_attestation_nonce_is_missing(self):
        client = self.build_client()
        client.usdid_registered = True
        ...
        with (
            ...
            result = await client.bloks_caa_login_prepare(username="override_user")
    ...
    async def test_send_login_auto_prepares_before_password_and_flow_selection(self):
        client = self.build_client()
        order = []
        ...
        client.password_encrypt = AsyncMock(side_effect=encrypt)
        ...
        with (
            ...
            result = await client.bloks_caa_login_send_request(
                "dummy_password",
                domain="b.i.instagram.com",
                offline_experiment_group="experiment-1",
                bloks_versioning_id="bloks-version-1",
        ...
        login_prepare.assert_awaited_once_with(
    ...
    async def test_send_login_uses_aac_even_when_prepare_returns_false(self):
        client = self.build_client()
        ...
        with (
            ...
            result = await client.bloks_caa_login_send_request("dummy_password")

        self.assertEqual(result, {"status": "ok"})
        action.assert_awaited_once()

    async def test_send_login_requires_aac_after_auto_prepare(self):
        client = self.build_client()
        client.password_encrypt = AsyncMock(return_value="#PWD_INSTAGRAM:4:1:encrypted")
        ...
        with (
            ...
            await client.bloks_caa_login_send_request(
                "dummy_password",
                username="override_user",
                waterfall_id="waterfall-1",
            )
        ...
        client.password_encrypt.assert_not_awaited()
        action.assert_not_awaited()

    async def test_send_login_rolls_back_failed_prepare_and_retries(self):
        client = self.build_client()
        error = RuntimeError("preflight failed")
        client.caa_waterfall_id = "previous-waterfall"
        client.attestation_challenge_nonce = "previous-challenge"
        client.attestation_key_nonce = "previous-key"
        client.password_encrypt = AsyncMock(return_value="#PWD_INSTAGRAM:4:1:encrypted")
        ...
        with (
            ...
            for _ in range(2):
                with self.assertRaises(RuntimeError) as cm:
                    await client.bloks_caa_login_send_request("dummy_password")

                self.assertIs(cm.exception, error)
                self.assertEqual(client.caa_aac, "")
                self.assertEqual(client.caa_waterfall_id, "previous-waterfall")
                self.assertEqual(client.attestation_challenge_nonce, "previous-challenge")
                self.assertEqual(client.attestation_key_nonce, "previous-key")

        self.assertEqual(login_prepare.await_count, 2)
        client.password_encrypt.assert_not_awaited()
        action.assert_not_awaited()

    async def test_send_login_rolls_back_cancelled_prepare_and_retries(self):
        client = self.build_client()
        client.password_encrypt = AsyncMock(return_value="#PWD_INSTAGRAM:4:1:encrypted")
        ...
        with mock.patch.object(client, "bloks_caa_login_prepare", side_effect=prepare) as login_prepare:
            for _ in range(2):
    ...
    async def test_send_login_can_disable_auto_prepare(self):
        client = self.build_client()
        client.password_encrypt = AsyncMock(return_value="#PWD_INSTAGRAM:4:1:encrypted")
        ...
        with (
            ...
            await client.bloks_caa_login_send_request("dummy_password", auto_prepare=False)

        prepare.assert_not_awaited()
        client.password_encrypt.assert_not_awaited()
        action.assert_not_awaited()

    async def test_send_login_prepares_only_once_across_repeated_calls(self):
        client = self.build_client()
        ...
        login_prepare.assert_awaited_once()
        self.assertEqual(action.await_count, 2)

    async def test_send_login_uses_server_aac_and_attestation_only_on_final_request(self):
        client = self.build_client()
        ...
        with (
            ...
            await client.bloks_caa_login_send_request("dummy_password", domain="b.i.instagram.com")
    ...
    async def test_send_login_accepts_encrypted_password_and_explicit_overrides_without_attestation(self):
        client = self.build_client()
        client.caa_aac = "server-aac"
        client.password_encrypt = AsyncMock(
            side_effect=AssertionError("encrypted passwords must not be encrypted twice")
        )
    ...
    async def test_full_caa_login_applies_direct_embedded_login_response(self):
        client = self.build_client()
        ...
        with (
            ...
            outcome = await client.bloks_caa_login()
    ...
    async def test_full_caa_login_returns_reason_when_preflight_is_incomplete(self):
        client = self.build_client()
    ...
    async def test_full_caa_login_can_skip_preflight_for_prepared_client(self):
        client = self.build_client()
        ...
        with (
            ...
            outcome = await client.bloks_caa_login(prepare=False)
    ...
    async def test_full_caa_login_dispatches_profile_code_challenge(self):
        client = self.build_client()
        ...
        with (
            ...
            outcome = await client.bloks_caa_login(verification_code="654321")
    ...
    async def test_full_caa_login_returns_legacy_context_when_no_session_is_applied(self):
        client = self.build_client()
...
class CaaVerifyProfileRegressionTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_context_extraction_matches_exact_app_and_reads_template_map(self):
        client = self.build_client()
        result = {
            "layout": {
                "bloks_payload": {
    ...
    async def test_context_extraction_pairs_context_key_with_value_at_same_index(self):
        client = self.build_client()
        result = {
            "layout": {
                "bloks_payload": {
    ...
    async def test_context_extraction_stays_in_target_string_and_decodes_escapes(self):
        client = self.build_client()
    ...
    async def test_context_extraction_does_not_use_a_later_apps_map(self):
        client = self.build_client()
        result = {
            "layout": {
                "bloks_payload": {
    ...
    async def test_context_extraction_returns_empty_for_missing_anchor_or_context(self):
        client = self.build_client()
    ...
    async def test_verify_profile_chains_contexts_and_applies_terminal_login(self):
        client = self.build_client()
        ...
        with (
            ...
            outcome = await client.bloks_caa_resolve_two_step_verification(send_result, verification_code="654321")
    ...
    async def test_verify_profile_uses_existing_challenge_code_handler(self):
        client = self.build_client()
        send_result = self.action(self.ENTRYPOINT, "entry-context")
        entry_result = self.action(self.CODE_ENTRY, "code-context")
        code_result = self.action(self.CODE_ENTRY_ASYNC, "submit-context", container="ft")
        ...
        with (
            ...
            await client.bloks_caa_resolve_two_step_verification(send_result)

        client.challenge_code_handler.assert_called_once()
        self.assertEqual(submit.call_args.args[:2], ("submit-context", "123456"))

    async def test_verify_profile_reports_each_missing_context(self):
        client = self.build_client()
        send_result = self.action(self.ENTRYPOINT, "entry-context")
        entry_result = self.action(self.CODE_ENTRY, "code-context")

        self.assertEqual(
            (await client.bloks_caa_resolve_two_step_verification({}))["reason"],
            "missing entrypoint context_data",
        )
        with mock.patch.object(client, "bloks_ap_two_step_verification_entrypoint", return_value={}):
            self.assertEqual(
                (await client.bloks_caa_resolve_two_step_verification(send_result))["reason"],
    ...
    async def test_verify_profile_raises_when_code_handler_returns_no_code(self):
        client = self.build_client()
        send_result = self.action(self.ENTRYPOINT, "entry-context")
        entry_result = self.action(self.CODE_ENTRY, "code-context")
        code_result = self.action(self.CODE_ENTRY_ASYNC, "submit-context", container="ft")
        client.challenge_code_handler = AsyncMock(return_value=None)
    ...
    async def test_verify_profile_surfaces_submit_code_rejection_as_challenge_error(self):
        client = self.build_client()
        send_result = self.action(self.ENTRYPOINT, "entry-context")
        entry_result = self.action(self.CODE_ENTRY, "code-context")
        code_result = self.action(self.CODE_ENTRY_ASYNC, "submit-context", container="ft")
        ...
        with (
            ...
            await client.bloks_caa_resolve_two_step_verification(send_result, verification_code="654321")

        self.assertIn("profile-code submission failed", str(raised.exception))
        self.assertIs(raised.exception.__cause__, rejection)

    async def test_verify_profile_requests_are_prelogin_and_use_caa_domain(self):
        client = self.build_client()

        with mock.patch.object(client, "bloks_async_action", return_value={}) as action:
            await client.bloks_ap_two_step_verification_entrypoint("entry-context", domain="b.i.instagram.com")
            await client.bloks_ap_two_step_verification_submit_code(
                "submit-context", "123456", domain="b.i.instagram.com"
            )
        ...
        with mock.patch.object(client, "bloks_app", return_value={}) as app:
            await client.bloks_ap_two_step_verification_code_entry("code-context", domain="b.i.instagram.com")
