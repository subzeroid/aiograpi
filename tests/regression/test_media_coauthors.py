...
class MediaCoauthorsTimelineRegressionTestCase(IsolatedAsyncioTestCase):
    async def test_user_medias_gql_accepts_null_coauthors_without_mutating_timeline(self):
        client = Client()
        ...
        client.private_graphql_request = mock.AsyncMock(return_value=response)
        client._user_medias_chunk_public_gql = mock.AsyncMock(
            side_effect=AssertionError("The app timeline should not require a public fallback")
        )

        medias = await client.user_medias_gql("2", amount=1)
