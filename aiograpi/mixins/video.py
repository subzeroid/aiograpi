...
class DownloadVideoMixin(ClientMixin):
    ...
    async def video_download(self, media_pk: int, folder: Path = "", overwrite: bool = True) -> Path:
        """
        ...
        return await self.video_download_by_url(media.video_url, filename, folder, overwrite=overwrite)
    ...
    async def video_download_by_url_origin(self, url: str) -> bytes:
        """
        Download video using URL
...
class UploadVideoMixin(ClientMixin):
    ...
    async def video_upload(
        ...
        for attempt in range(50):
            ...
            try:
                configured = await self.video_configure(
                    upload_id,
                    width,
                    height,
                    duration,
    ...
    async def video_configure(
        ...
        return await self.private_request("media/configure/?video=1", self.with_default_data(data))
    ...
    async def video_configure_to_story(
        ...
        return await self.private_request("media/configure_to_story/?video=1", self.with_default_data(data))
