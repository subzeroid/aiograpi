...
class UserMixin(ClientMixin):
    ...
    async def user_following_private_gql_chunk(
        self,
        user_id: str,
        max_amount: int = 0,
    ...
    async def user_following_private_gql(
        self,
        user_id: str,
        amount: int = 0,
        rank_token: Optional[str] = None,
        ...
        while True:
            ...
            chunk, max_id = await self.user_following_private_gql_chunk(
                user_id,
            ...
            if self._has_private_auth():
                try:
                    ...
                    if not isinstance(e, ClientError):
                        self.logger.exception(e)
                    try:
                        users = await self.user_following_private_gql(user_id, amount)
                    except Exception as e:
                        if not isinstance(e, ClientError):
                            self.logger.exception(e)
    ...
    async def user_followers(
        ...
        if not use_cache or not users or (amount and len(users) < amount):
            if self._has_private_auth():
                ...
                if limited:
                    try:
                        users = await self.user_followers_private_gql(user_id, amount)
                    except Exception as e:
                        if not isinstance(e, ClientError):
                            self.logger.exception(e)
