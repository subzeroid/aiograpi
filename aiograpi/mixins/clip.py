...
class ClipMixin(ClientMixin):
    ...
    async def clip_seen(self, media_ids: List[str], blend_media_ids: Optional[List[str]] = None) -> bool:
        """
...
class UploadClipMixin(ClientMixin):
    ...
    async def clip_info_for_creation(self, device_status: Optional[Dict[str, object]] = None) -> Dict:
        """
    ...
    async def clip_interest_topics(self) -> List[Dict[str, object]]:
        """
    ...
    async def _clip_share_to_fb_unified_config_variants(self):
        yield await self.clip_share_to_fb_unified_config()
        for surface_list in CLIP_FB_CROSSPOSTING_SURFACE_FALLBACKS:
            yield await self.clip_share_to_fb_unified_config(crosspost_app_surface_list=surface_list)
    ...
    async def clip_share_to_fb_unified_destination(
        ...
        async def resolve(unified_config: Dict[str, object]) -> Optional[Dict[str, object]]:
        ...
        if config is not None:
            destination = await resolve(config)
            if destination:
                return destination
        else:
            async for unified_config in self._clip_share_to_fb_unified_config_variants():
    ...
    async def clip_upload(
        ...
        if share_to_threads:
            threads_extra_data = await self.media_share_to_threads_extra_data(
        ...
        for attempt in range(50):
            ...
            try:
                configured = await self.clip_configure(
                    upload_id,
                    thumbnail,
                    width,
                    height,
                ...
                "clips_segments": [
                    {
                        ...
                        "original_segment_hash": None,
                        "is_remix": False,
                        "has_reusable_template_asset": False,
                    }
                ],
            },
            "clips_audio_metadata": {"original": {"volume_level": 1.0}},
