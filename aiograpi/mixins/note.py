...
class NoteMixin(ClientMixin):
    ...
    async def last_seen_update_note(self) -> bool:
        """
    ...
    async def notes_music_browser(self) -> Dict:
        """
    ...
    async def create_music_note(
        ...
        if not alacorn_session_id:
            alacorn_session_id = (await self.notes_music_browser()).get("alacorn_session_id")
        ...
        result = await self.private_graphql_request(data)
