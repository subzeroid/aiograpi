...
class DownloadAlbumMixin(ClientMixin):
    ...
    async def album_download_origin(self, media_pk: int) -> List[bytes]:
        """
        Download your album
...
class UploadAlbumMixin(ClientMixin):
    async def album_upload(
        ...
        for path in paths:
            ...
            if path.suffix.lower() in (".jpg", ".jpeg", ".png", ".webp"):
                ...
                (
                    ...
                    duration,
                    thumbnail,
                ) = await self.video_rupload(path, to_album=True)
                children.append(
                    {
                        "upload_id": upload_id,
        ...
        for attempt in range(50):
            ...
            try:
                configured = await (configure_handler or self.album_configure)(
    ...
    async def album_upload_with_music(
        self,
        paths: List[Path],
        caption: str,
        ...
        data["music_params"] = await self._feed_music_params(
            track,
        ...
        return await self.album_upload(
            paths,
            caption,
            usertags=usertags,
            location=location,
