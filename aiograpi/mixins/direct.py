...
class DirectMixin:
    ...
    async def direct_pending_inbox(self, amount: int = 20) -> List[DirectThread]:
        ...
        while True:
            ...
            result = await self.private_request(
                "direct_v2/pending_inbox/", params=params
            )
            inbox = result.get("inbox", {})
            for thread in inbox.get("threads", []):
                threads.append(extract_direct_thread(thread))
    ...
    async def direct_send_video(
        ...
        """
        return await self.direct_send_file(
            path, user_ids, thread_ids, content_type="video"
        )
    ...
    async def direct_thread_hide(self, thread_id: int) -> bool:
        ...
        result = await self.private_request(
            f"direct_v2/threads/{thread_id}/hide/", data=data
        )
        return result["status"] == "ok"

    async def direct_media_share(
        self, media_id: str, user_ids: List[int]
    ) -> DirectMessage:
        """
