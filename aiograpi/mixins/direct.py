...
from aiograpi.utils.video import read_video_metadata, read_video_metadata_with_moviepy
...
class DirectMixin(ClientMixin):
    ...
    async def direct_pending_inbox(self, amount: int = 20) -> List[DirectThread]:
        ...
        threads = []
        while True:
            new_threads, cursor = await self.direct_pending_chunk(cursor)
    ...
    async def direct_pending_requests_preview(self, pending_inbox_filters: Optional[List[str]] = None) -> Dict:
        """
        ...
        return await self.private_request("direct_v2/async_get_pending_requests_preview/", params=params)
    ...
    async def direct_pending_approve(self, thread_id: int) -> bool:
        """
    ...
    async def direct_spam_inbox(self, amount: int = 20) -> List[DirectThread]:
        ...
        threads = []
        while True:
            new_threads, cursor = await self.direct_spam_chunk(cursor)
            for thread in new_threads:
                threads.append(thread)
    ...
    async def direct_spam_chunk(self, cursor: str = None) -> Tuple[List[DirectThread], str]:
        ...
        threads = []
        result = await self.private_request("direct_v2/spam_inbox/", params=params)
        inbox = result.get("inbox", {})
        for thread in inbox.get("threads", []):
            threads.append(extract_direct_thread(thread))
        cursor = inbox.get("oldest_cursor")
    ...
    async def _direct_thread_id_from_user_ids(self, user_ids: List[int], media_kind: str) -> int:
        user_ids = _direct_id_list(user_ids)
    ...
    async def direct_send_video(
        ...
        Replicates the IG Android client's current three-step protocol:
        upload the MP4 bytes to ``rupload.facebook.com/messenger_video``,
        then broadcast ``direct_v2/threads/broadcast/raven_attachment/?video=1``
        ...
        if user_ids:
            thread_ids = [await self._direct_thread_id_from_user_ids(user_ids, "video")]
        ...
        media_id = await self._video_rupload(video_bytes, entity, waterfall_id)
    ...
    def _messenger_rupload_headers(self, extra_headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        ...
        headers = {
            ...
            # The real client may send zstd; rupload accepts gzip for these
            # Direct attachment flows.
    ...
    async def _video_rupload(self, video_bytes: bytes, entity_name: str, waterfall_id: str) -> int:
        ...
        try:
            offset = int(response.json().get("offset", 0))
        except Exception:
            offset = 0
        ...
        if response.status_code != 200:
            raise ClientError(f"messenger_video upload POST failed: {response.status_code} {response.text[:300]}")
        try:
            return int(response.json()["media_id"])
        except Exception as exc:
            raise ClientError(f"messenger_video response missing media_id: {response.text[:300]}") from exc
    ...
    async def direct_send_voice(
        ...
        The audio file MUST be AAC in an MP4 container (``.m4a``).
        """
        assert self.user_id, "Login required"
        user_ids = _direct_id_list(user_ids)
        thread_ids = _direct_id_list(thread_ids)
        ...
        if user_ids:
            thread_ids = [await self._direct_thread_id_from_user_ids(user_ids, "voice")]

        audio_bytes = Path(path).read_bytes()
        upload_id = str(int(time.time() * 1000))
        rand_key = random.randint(-(2**31), 2**31 - 1)
        media_id = await self._voice_rupload(audio_bytes, upload_id, rand_key)
    ...
    async def _voice_rupload(self, audio_bytes: bytes, upload_id: str, rand_key: int) -> int:
        """Upload m4a bytes to ``rupload.facebook.com/messenger_audio/...``."""
        ...
        if response.status_code != 200:
            raise ClientError(f"messenger_audio upload POST failed: {response.status_code} {response.text[:300]}")
        try:
            return int(response.json()["media_id"])
        except Exception as exc:
            raise ClientError(f"messenger_audio response missing media_id: {response.text[:300]}") from exc
    ...
    async def direct_users_presence(self, user_ids: List[int]) -> Dict:
        """
    ...
    async def direct_message_seen(self, thread_id: int, message_id: int) -> bool:
        """
    ...
    async def direct_send_seen(self, thread_id: int) -> bool:
        """
    ...
    async def direct_has_interop_upgraded(self) -> bool:
        """
        ...
        """
        assert self.user_id, "Login required"
        result = await self.private_request("direct_v2/has_interop_upgraded/")
        return bool(result.get("has_interop_upgraded"))

    async def direct_search_gen_ai_bots(self, amount: int = 20) -> List[UserShort]:
        """
    ...
    async def direct_channels(
        self, user_id: Optional[int] = None, thread_subtypes: Optional[List[int]] = None
    ) -> List[Dict]:
        """
        ...
        result = await self.private_request("direct_v2/get_all_channels/", params=params)
        return result.get("all_channels_list", [])

    async def direct_thread_by_participants(self, user_ids: List[int]) -> Dict:
        """
    ...
    async def direct_set_e2ee_eligibility(self, e2ee_eligibility: int = 4) -> bool:
        """
    ...
    async def direct_thread_unmute_video_call(self, thread_id: int) -> bool:
        ...
        bool
            A boolean value
        """
        return await self.direct_thread_mute_video_call(thread_id, revert=True)
    ...
    async def direct_media(self, thread_id: int, amount: int = 20) -> List[Media]:
        """
