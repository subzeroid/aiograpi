...
class TOTPMixin(ClientMixin):
    async def totp_generate_seed(self) -> str:
        ...
        """
        result = await self.private_request("accounts/generate_two_factor_totp_key/", data=self.with_default_data({}))
    ...
    async def totp_disable(self) -> bool:
        ...
        bool
        """
        result = await self.private_request("accounts/disable_totp_two_factor/", data=self.with_default_data({}))
