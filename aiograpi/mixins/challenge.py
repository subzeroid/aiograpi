...
def extract_messages(challenge):
    messages = []
    extra_data = challenge.get("extraData") or {}
    for item in extra_data.get("content") or []:
...
class ChallengeResolveMixin(ClientMixin):
    ...
    async def _challenge_resolve_change_password(self) -> bool:
        ...
        return await self.bloks_change_password(pwd, challenge_context)

    async def challenge_bloks_redirect_dismiss(self) -> bool:
        """
    ...
    async def challenge_resolve_contact_form(self, challenge_url: str) -> bool:
        ...
        try:
            ...
            for key, value in self.private.cookies_dict().items():
                if key in ["mid", "csrftoken"]:
                    session.cookies.set(key, value)
            await asyncio.sleep(WAIT_SECONDS)
            result = await session.get(challenge_url)  # render html form
            session.headers.update(
                {
                    "x-ig-www-claim": "0",
                    ...
                    "x-csrftoken": session.cookies_dict().get("csrftoken"),
            ...
            for retry_code in range(5):
                ...
                # SEND CODE
                await asyncio.sleep(WAIT_SECONDS)
                result = (await session.post(challenge_url, data={"security_code": code})).json()
    ...
    async def challenge_resolve_simple(self, challenge_url: str) -> bool:
        ...
        if step_name == "delta_login_review" or step_name == "scraping_warning":
            ...
            if step_name == "select_verify_method":
                ...
                if "email" in steps:
                    choice = ChallengeChoice.EMAIL
                    await self._send_private_request(challenge_url, {"choice": str(choice.value)})
            ...
            if self.last_json.get("challenge_type_enum_str") == "PASSWORD_RESET":
                return await self._challenge_resolve_change_password()
            challenge_context = self.last_json.get("challenge_context")
            if not challenge_context:
                self._raise_bloks_redirect_required()
            result = await self.bloks_challenge_take_challenge(
