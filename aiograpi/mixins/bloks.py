...
from aiograpi.exceptions import ChallengeError, ClientError
...
class BloksMixin(ClientMixin):
    ...
    async def bloks_fxcal_link_reels_share(
        ...
        return await self.bloks_async_action(
            "com.bloks.www.fxcal.link.async",
            params,
            bloks_versioning_id=bloks_versioning_id,
        )

    async def bloks_two_step_verification_entrypoint(
        self,
    ...
    async def bloks_caa_login_process_client_data(
        self,
        waterfall_id: str = "",
        ...
        result = await self.bloks_async_action(
    ...
    async def bloks_caa_login_oauth_token_fetch(
        self,
        username: str = "",
        waterfall_id: str = "",
        offline_experiment_group: str = "caa_iteration_v3_perf_ig_4",
    ...
    async def bloks_caa_login(
        ...
        if prepare and not (await self.bloks_caa_login_prepare(username=username, domain=domain)):
            return {
                "logged_in": False,
        ...
        if not logged_in and self.bloks_caa_login_needs_two_step(result):
            two_step = await self.bloks_caa_resolve_two_step_verification(
                result,
    ...
    async def bloks_ap_two_step_verification_entrypoint(
        self,
        context_data: str,
        bloks_versioning_id: str = "",
        domain: Optional[str] = None,
    ...
    async def bloks_ap_two_step_verification_code_entry(
        self,
        context_data: str,
        bloks_versioning_id: str = "",
        domain: Optional[str] = None,
    ...
    async def bloks_ap_two_step_verification_submit_code(
        self,
        context_data: str,
        code: str,
        bloks_versioning_id: str = "",
    ...
    async def bloks_caa_resolve_two_step_verification(
        self,
        send_result: Dict,
        verification_code: str = "",
        domain: Optional[str] = None,
        ...
        entry_result = await self.bloks_ap_two_step_verification_entrypoint(entry_context, domain=domain)
        ...
        code_result = await self.bloks_ap_two_step_verification_code_entry(code_context, domain=domain)
        ...
        code = verification_code or (await self.challenge_code_or_raised(ChallengeChoice.EMAIL))
        try:
            submit_result = await self.bloks_ap_two_step_verification_submit_code(
                submit_context,
                code,
                domain=domain,
            )
    ...
    def bloks_apply_login_response(self, result: Dict) -> bool:
        ...
        if cookies:
            self.private.set_cookies(cookies)
        if cookies.get("sessionid"):
            self.public.set_cookies({"sessionid": cookies["sessionid"]})
