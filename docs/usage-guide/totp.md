...
The previous login flow is available explicitly as `await cl.login_legacy(...)`, with the same arguments and its existing CAA/Bloks fallbacks. Default `login()` enters the legacy flow only when Instagram explicitly returns a `CAA_LOGIN_FALLBACK` instruction. See the [login migration guide](login-migration.md).
