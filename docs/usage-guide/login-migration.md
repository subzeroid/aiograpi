...
`login()` uses CAA directly. Since aiograpi 2.0.3, an explicit `CAA_LOGIN_FALLBACK` instruction from Instagram continues through `login_legacy()`, allowing the accounts endpoint to complete login or surface its typed error. Other CAA failures retain their normal error handling. All private mobile API requests use curl with HTTP/2 by default. Public web and GraphQL transports retain their separate configuration.
