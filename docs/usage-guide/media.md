...
| clip_upload(path: Path, caption: str, thumbnail: Path, usertags: List[Usertag], location: Location, extra_data: Dict = {}, trial: bool = False, share_to_facebook: bool = False, topics: List[int \| str] = None, show_preview_in_feed: bool = True) | Media | Upload Reels Clip (Support MP4 files), optionally as a Trial Reel, cross-posted to Facebook, published with Reel topic `fit_id` values, or hidden from the feed/profile grid preview
