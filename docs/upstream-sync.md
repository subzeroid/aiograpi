...
The next `aiograpi` feature release continues the mirror through `instagrapi 2.9.0`. It adds the experimental modern CAA email signup flow via `signup_caa_email(...)`, the `graphql_www` Bloks app wrapper used by registration, and per-request private headers/domain routing so Bloks friendly names do not leak onto unrelated private requests.
