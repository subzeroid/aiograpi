...
- Normalize carousel resource IDs in app profile timeline responses before extraction, so `user_medias_gql()` accepts children with a composite `id` and no `pk` (#479; mirrors instagrapi #2831). Explicit resource IDs and validation of identifier-free children are preserved.

### Changed

- Record synchronization through instagrapi 3.0.13, including the carousel resource ID fix.
...
- `login()` now follows Instagram's CAA fallback instruction (`CAA_LOGIN_FALLBACK:fallback_triggered`) and completes the legacy accounts flow, so typed failure reasons such as `PleaseWaitFewMinutes` or `BadPassword` surface instead of the generic "CAA login did not return a session" error. `ClientError` raised by CAA login carries the observed CAA step markers as `caa_actions` for diagnostics.
...
- Recorded compatibility through `instagrapi` 2.18.18 and added real HTTPX transport regression coverage confirming that private HTTP 429 responses remain typed without hidden retries while public requests use exactly their configured outer retry loop.
- Clarified that private `PleaseWaitFewMinutes` responses are surfaced immediately, unlike the configurable public retry path.
